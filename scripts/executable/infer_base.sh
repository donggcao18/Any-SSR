...
deepspeed --master_port "$port" training/main_anamoe.py \
  ...
  --num_test -1 \
  --run_name run_1 \
  --group_name infer_base_Qwen2.5-Coder-1.5B_executable \
  --max_prompt_len 1024,1024,1024,1024,1024,1024,1024,1024,1024 \
  --max_ans_len 2048,2048,2048,2048,2048,2048,2048,2048,2048 \
  --num_train_epochs 3

: "${HF_MODEL_REPO_ID:=ankhanhtran02/infer_base_Qwen2.5-Coder-1.5B_executable}"

python upload_output_to_hf.py \
  --output-dir "./output_models/infer_base_Qwen2.5-Coder-1.5B_executable" \
  --repo-id "$HF_MODEL_REPO_ID" \
  --commit-message "Upload infer_base executable outputs"